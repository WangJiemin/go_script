...
func (e *flags) init() {
	...
	if flag.NFlag() == 0 {
		fmt.Println("请输入【-h】查看帮助！\n\tSample :shell> nohup ./godba -lazy -d sda -C 5 -i 2 -L /tmp/godba.log  > /dev/null 2>&1 &")
...
func createCommand(info map[string]interface{}, count int) basic {
	...
	if info["rt"] == true {
		if count == 0 {
			...
			var rt_cmd string
			rt_cmd = "tail -1 /tmp/godba_tcprstat.log |awk '{print $2,$5,$9,$12}'|sed 's/[[:space:]]/,/g'"
...
func gotNumber(flag_info map[string]interface{}, first basic, second basic, count int) {
	...
	if flag_info["subordinate"] == true {
		title_summit += Colorize("----------- Subordinate Status --------", green, blue, "", "")
...
func add_log(flag_log map[string]interface{}, info string) {
	...
	if flag_log["logfile_by_day"].(bool) == true {
		...
		if flag_log["logfile"].(string) != "none" {
			...
			file_name = "/tmp/godba" + "_" + fmt.Sprintf("%s", t.Format("2006-01-02")) + ".log"
		}
	} else {
		if flag_log["logfile"].(string) != "none" {
			file_name = flag_log["logfile"].(string)
		} else {
			file_name = "/tmp/godba.log"
		}
	}
...
func main() {
	...
	if info["rt"] == true {
		go func() {
			var rt_cmd string
			rt_cmd = "tcprstat --no-header -p " + info["port"].(string) + " -t " + info["interval"].(string) + " -n 0 -l `/sbin/ifconfig | grep 'addr:[^ ]\\+' -o | cut -f 2 -d : | xargs echo | sed -e 's/ /,/g'` 1>/tmp/godba_tcprstat.log"
			execCommand(rt_cmd)
		}()
	}
