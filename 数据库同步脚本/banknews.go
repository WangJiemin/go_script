...
type NEWS struct {
	...
	//READSTATUS       string `gorm:"cloumn:readStatus"`
}
...
func main() {
	...
	/*
		...
		db124.Table("funbird_analysis.funbird_table").Where("tablename = ?", "information_all_5e619c69").Select("numid").Find(&funbridtable)
		for _, v := range funbridtable {
			numid = v.NUMID
		}
		fmt.Printf("update table name is:\t%s\tAUTO INCREMENT number is:\t%d\n", "information_all_5e619c69", numid)
	*/

	inforall5e := []*InForMationAll{}

	//db124.Table("information_all_5e619c69").Where("id > ? and title IS NOT NULL and title != '' and content IS NOT NULL and content != ''", numid).Find(&inforall5e)
	db124.Table(fmt.Sprintf("%s_%s", "information_all_5e619c69", yesterday)).Where("title IS NOT NULL and title != '' and content IS NOT NULL and content != ''").Find(&inforall5e)

	for _, v := range inforall5e {
		//fmt.Print(v)
		bannew := NEWS{}
		bannew.ID = v.Leaf_id
		bannew.URL = v.Url
		bannew.TITLE = v.Title
		bannew.AUTHOR = v.Author
		bannew.SOURCE = v.Source
		bannew.RELEASE_DATETIME = v.Pub_time
		bannew.CONTENT = v.Content
		bannew.MEDIA_TYPE = v.Media_type
		bannew.ORIGINAL_TITLE = v.Original_title
		bannew.EDITOR = v.Editor
		bannew.REPORTER = v.Reporter
		bannew.CONTENTS = v.Contents
		bannew.READING_TIMES = v.Reading_times
		bannew.ABSTRACT_DATA = v.Abstract_data
		bannew.MEDIA = v.Media
		bannew.MEDIA_CHANNEL = v.Media_channel
		bannew.LOCATION = v.Location
		bannew.LOCATIONPATH = v.Location_path
		bannew.COLLECTION_TOOL = v.Collection_tool
		bannew.USER = v.User
		bannew.SITE_URL = v.Site_url
		bannew.Leafid = v.Leaf_id
		bannew.Taskid = v.Task_id
		bannew.TASKNAME = v.Task_name
		bannew.GET_TIME = v.Get_time
		bannew.KEYWORD = v.Keyword
		bannew.MARK = 0

		if v.Pub_time == "" {
			bannew.RELEASE_DATETIME = v.Get_time
		}
		db140.Table("banknews").Save(&bannew)
		//db124.Table("funbird_analysis.funbird_table").Where("tablename = ?", "information_all_5e619c69").Update("numid", v.Id)
	}
